...
**Still the old app shape:** two-tab `AppShell`, Seniority Live path, no profiles, no
...
| 2 | **Unified shell + session machine** — drop `AppShell`; `profiles.ts`; start screen (profile + Practice\|Simulation); grow practice loop into [020](tickets/020-unified-session-flow-ux.md) phase machine; progress chrome; End session | [020](tickets/020-unified-session-flow-ux.md), [019](tickets/019-interview-profile-config.md) |
